...
- **External-clock divider feel:** Story 010 ships with the tempo pot acting as ÷1 / ÷2 / ÷4 when external clock is active. Bench feedback (2026-04-23): only ÷1 sees real use — ÷2 and ÷4 audibly *slow* the arp, which is rarely the desired behaviour at Eurorack clock rates. Options to reconsider: (a) bias the pot toward ÷1 (e.g. bottom 2/3 = ÷1, then ÷2, then ÷4), (b) replace divider with multiplier (×1 / ×2 / ×4 — needs period interpolation), or (c) repurpose the pot in external mode (swing, gate length, octave shift). For now: leave as-is and revisit if it feels limiting in actual patches.
