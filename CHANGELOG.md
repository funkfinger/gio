...
- **Aseprite CLI auto-export pipeline.** New `tools/aseprite-export.js` walks `assets/screens/**/*.aseprite`, reads each file's tags via a tiny Lua script (`tools/aseprite-list-tags.lua`), and exports each **single-frame tag** as `<base>-<tagname>.png`. Multi-frame tags (e.g. a "Loop" covering all frames) are intentionally ignored — they're informational, not export targets. Untagged frames are also ignored, so existing manual PNGs (like the splash animation) stay untouched. Tag-based naming (`key-A.png`) makes the C++ lookup self-documenting (`screens::key_A`). Wired into `npm run build:screens` ahead of `bitmap2header.js`. Aseprite path defaults to the macOS standard install; override via `ASEPRITE_PATH`.
- **Quirk worked around:** Aseprite 1.3.17.1's `--list-tags` and `--data` JSON tag-export are broken (return empty even when tags exist). The Lua scripting API reads `sprite.tags` correctly, so the tool uses Lua for introspection and per-tag CLI invocations (`--frame-range`) for export. Documented inline.
- **Off-by-one note:** Aseprite's `--frame-range` is 0-indexed while the Lua API reports `frameNumber` 1-indexed. The export script subtracts 1 when crossing the boundary. Verified empirically.
