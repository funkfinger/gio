...
- **OLED on `Wire1`, not `Wire` (arduino-pico 5.6.0 default pin remap on XIAO RP2350).** `firmware/arp/lib/oled_ui/oled_ui.h` now instantiates `Adafruit_SSD1306 display_{… &Wire1, -1}` instead of `&Wire`. **Why:** arduino-pico 5.6.0 changed the seeed_xiao_rp2350 variant's default Wire (Wire0) pin mapping from `GP6/GP7` (= `D4/D5`, where the gio board's OLED is wired) to `GP16/GP17` (unused on the gio board). GP6/GP7 are now `PIN_WIRE1_SDA/SCL`, so the OLED lives on `Wire1` and `Wire1`'s defaults match the gio wiring (no `setSDA`/`setSCL` remap needed). Symptom before the fix: `ui.begin()` reported `ready=true` (Adafruit_SSD1306 doesn't actually round-trip an I²C ACK during begin), but the OLED never displayed anything — the library was writing to a peripheral with nothing on its bus. Full chase in `bench-log.md` 2026-05-23 entry.
- **arduino-pico framework upgraded `4.4.0` → `5.6.0` in `firmware/arp/platformio.ini`.** Mid-session, builds against the pinned 4.4.0 began producing UF2s that don't boot on stock XIAO RP2350 boards (USB CDC never enumerates, no serial output, OLED dark — completely silent fail). Old 4.4.0 builds already on boards still worked; *new* builds from the same source did not. Root cause never pinned down — likely a transitive dependency drift somewhere in the PlatformIO/platform-raspberrypi/picotool chain since `platform = …main` is unpinned. Upgrading the framework pin to 5.6.0 restored bootable UF2 output. **Followup pending:** pin `platform = …#<commit>` and `platform_packages` for picotool to specific known-good versions so this can't happen silently again.

### Changed

- **`OLED_ROTATION` reverted to `0` (native landscape)** for board #2's OLED orientation. The rev 1 PCB OLED footprint bug is per-board: board #1 needed a 180° physical flip and `OLED_ROTATION = 2`; board #2 happens to land at `ROTATION = 0` for the way its OLED was installed. **Rev 2 to-do**: fix the OLED footprint in KiCad so all boards work with a single canonical rotation (probably `1` = 90° CW portrait, the original design intent). Affects both production and smoke-test builds (shared header).
- **`firmware/arp/src/main.cpp` setup() reordered** — `Serial.begin()` now runs *before* `Wire.begin()` / `SPI.begin()` / any peripheral init. Previous ordering hid early-setup hangs behind silent USB CDC (no print output ever flushed), which made the 2026-05-23 toolchain regression several hours harder to isolate than it needed to be. Side effect: `Serial.println("BOOT N: <step>")` diagnostics added between every begin() call, plus a 1 Hz `LOOP heartbeat t=<ms>  ui.ready=<0|1>` print in loop(), so any future setup() hang is immediately diagnosable from a serial capture alone. **TEMP**: marked as diagnostic in the comments; consider keeping permanently under a `#ifdef DEBUG_BOOT` once we trust the rebuild path.

### Added

- **`firmware/arp/src/main_minimal.cpp` + `env:seeed-xiao-rp2350-minimal`** — a deliberate "absolute minimum sketch" build with no libraries, no peripherals, no SPI/Wire, just `Serial.begin()` + a `Serial.println("alive t=N")` loop. Built during the 2026-05-23 toolchain regression chase to isolate "is it our code or the build environment?" Kept around as a permanent recovery / sanity-check tool: if this env's UF2 doesn't boot, the rest of the firmware certainly won't and the problem isn't our code. Build: `pio run -d firmware/arp -e seeed-xiao-rp2350-minimal --target upload`.

### Docs

- **`docs/bench-log.md` 2026-05-21 entry — full electrical bring-up of the manufactured rev 1 board.** Captures: per-subsystem validation (XIAO + USB + Eurorack +5 V, SPI bus, DAC8552, MCP3208, REF3040, both input op-amp stages, both output op-amp stages, tempo pot, encoder rotation + click + long-press, OLED post-rework); the OLED footprint bug + 180° rework + `OLED_ROTATION=2` firmware bodge; the 2×2 patch matrix (Input A/B × Output A/B loopback); a productive false-alarm chase through the input op-amp transfer function (the apparent "−1.8 V output DC offset" was a missing 1.18× bias multiplier — `V_adc = (1 + R_fb/R_in_eq)·V_bias − (R_fb/R_in_eq)·V_jack`, not the simpler form I was treating it as); multimeter measurements confirming all four bias dividers are at design 1.66 V; and the rev 2 PCB to-do list. **Conclusion: the board is electrically fully validated, no PCB rework needed beyond the OLED flip.** Lesson added to the troubleshooting playbook: cross-check apparent DC offsets at an ADC channel against the floating reading before chasing a divider hypothesis.

### Added

- **Output op-amp stage (U5) generated into `hardware/gio-opamp/gio-opamp.kicad_sch`** via Python script (`tools/gen-output-stage.py`). The script translates the input op-amp section by +100 in Y and remaps references / values / net labels to the output topology. Adds: U5 (TL072 with 3 unit instances), R13–R22 (R_fb 47k ×2, R_jack_series 1k ×2, R_off_top 22k ×2, R_off_bot 14.7k ×2, R_in 10k ×2), C6–C10 (5× 100 nF for decoupling + bias filtering), D3/D4 (BAT54S output clamps), J3/J4 (PJ3001F output jacks), and 12 new power flags (#PWR022–#PWR033). Net labels: `dac_a`/`dac_b` (DAC inputs to R_in) and `out_a`/`out_b` (jack outputs after R_jack_series). **Generated by Claude — needs visual verification in KiCad.** Pin geometries empirically derived from the existing input stage's wire endpoints; UUIDs all freshly generated; paren balance verified; no duplicate UUIDs or designator collisions.
...
- **REF3040 voltage reference replaces the pot+TL072 VREF stand-in on the bench** (`bench-log.md` 2026-05-04 continued entry). VREF stability improved from **±56 mV** (pot+TL072 drift across a 4 s window) to **±2 mV** — a 28× improvement, essentially down to MCP3208 LSB quantization. Wiring per `bench-wiring.md` §5: REF3040 pin 1 (VIN) → +5 V, pin 2 (GND) → GND, pin 3 (VOUT) → VREF rail; 100 nF on VIN, 1 µF on VOUT (required for stability). TL072 #1 also pulled — REF3040's 25 mA output drives the ~1 mA total load directly. Bench now matches the production PCB topology for VREF. Existing calibration constants in `main.cpp` may shift slightly (well under 0.1 %, ratiometric system) and will be re-fit next calibration pass.
- **Bench platform fully revalidated end-to-end on the schematic-aligned layout** (`bench-log.md` 2026-05-04 entry). After rebuilding the breadboard's input scaling stage to match the KiCad schematic 1:1, both input channels (J1 and J2) and both output channels (J5 and J6) were validated via direct tip-to-tip jumper loopbacks: `j1_v` and `j2_v` each track the DAC triangle from 0.35 → 3.58 V at 1 Hz with errors under 30 mV across the full ±9 V swing. The pot on MCP3208 CH0, the encoder (now with A→D1/B→D2 natural wiring matching the schematic), and the OLED are all live. **The schematic is now a high-confidence PCB target** — every connection has been physically verified. Caught and documented one wiring failure mode along the way: the input chain was originally landed on op-amp pin 1 (output) instead of pin 2 (inverting input), producing a 4× gain error that mimicked the same symptom as a wrong R_fb value — now in the troubleshooting playbook.
- **Jack naming convention adopted from the KiCad schematic going forward**: J1 = Input A, J2 = Input B, J5 = Output A, J6 = Output B. (J3/J4 were used during early schematic iterations and KiCad's auto-annotation didn't reuse them.) All future docs and bench transcripts use these names.
- **Encoder A/B firmware swap reverted** (`PIN_ENC_A = D1`, `PIN_ENC_B = D2`) in both `src/main.cpp` and `src/main_smoketest.cpp` to align with the KiCad schematic's natural assignment. The 2026-05-02 firmware-side swap was a workaround for the bench encoder reading CW as a negative delta with the original wiring. On 2026-05-04 the breadboard wires were physically swapped to match the schematic, so firmware can now use the schematic-natural pin assignment. CW still reads positive — same observable behavior, just the responsibility for the swap moved from firmware to the breadboard. Convention now: encoder pin A → XIAO D1, encoder pin B → XIAO D2, common → GND. `bench-wiring.md` §3 + §8 + the encoder note all updated to agree. Build still 2.2 % RAM / 4.6 % flash; 66 host tests pass.
