...
- **`OLED_ROTATION` bumped from `1` to `2` in `firmware/arp/lib/oled_ui/oled_ui.h`** as a bodge for the gio rev 1 PCB's reversed OLED footprint (see `bench-log.md` 2026-05-21 entry). The 4-pin OLED header on rev 1 has VCC and GND swapped relative to the actual 0.49" SSD1306 module pinout — almost certainly cooked the IC on first power-on. Rework was to physically flip the OLED 180°; firmware compensation is +180° framebuffer rotation on top of the original `1` (90° CW portrait) → ends at `2` (180°). Text reads correctly post-rework. **Rev 2 to-do**: fix the OLED footprint in KiCad so the module sits in its natural orientation, and revert `OLED_ROTATION` to `1` on the rev 2 firmware. Affects both production and smoke-test builds (shared header).
